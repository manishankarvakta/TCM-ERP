...
export async function deleteLinePermanently(id: string) {
  try {
    const session = await auth();
    if (!session?.user) {
      return { success: false, error: "Unauthorized" };
    ...
    if (!canDelete) {
      return { success: false, error: "You don't have permission to delete lines permanently" };
    }
    ...
    // Check if any employee is associated with this line
    const employeeCount = await prisma.employee.count({
      where: { lineId: id },
    });
    if (employeeCount > 0) {
      return { success: false, error: `Cannot delete permanently: ${employeeCount} employees are associated with this line.` };
    }
    ...
    await logItemDeleted(session.user.id, "Line (Permanent)", id, oldLine.name);
    revalidateBothPaths("employees");

    return { success: true };
  } catch (error) {
    console.error("deleteLinePermanently error:", error);
    return { success: false, error: error instanceof Error ? error.message : "Failed to delete line permanently" };
  }
}

export async function bulkUpdateLineStatus(ids: string[], action: "trash" | "active" | "inactive" | "restore" | "delete-permanently") {
  try {
    const session = await auth();
    if (!session?.user) {
      return { success: false, error: "Unauthorized" };
    ...
    if (action === "delete-permanently") {
      ...
      if (employeeCount > 0) {
        return { success: false, error: `Cannot delete permanently: employees are associated with some of these lines.` };
      }
