...
export async function deleteEmployeeTypePermanently(id: string) {
  try {
    const session = await auth();
    if (!session?.user) {
      return { success: false, error: "Unauthorized" };
    ...
    if (!canDelete) {
      return { success: false, error: "You don't have permission to delete employee types permanently" };
    }
    ...
    // Check if any employee is associated with this type
    const employeeCount = await prisma.employee.count({
      where: { employeeTypeId: id },
    });
    if (employeeCount > 0) {
      return { success: false, error: `Cannot delete permanently: ${employeeCount} employees are associated with this type.` };
    }
    ...
    await logItemDeleted(session.user.id, "EmployeeType (Permanent)", id, oldEmployeeType.name);
    revalidateBothPaths("employees");

    return { success: true };
  } catch (error) {
    console.error("deleteEmployeeTypePermanently error:", error);
    return { success: false, error: error instanceof Error ? error.message : "Failed to delete employee type permanently" };
  }
}

export async function bulkUpdateEmployeeTypeStatus(ids: string[], action: "trash" | "active" | "inactive" | "restore" | "delete-permanently") {
  try {
    const session = await auth();
    if (!session?.user) {
      return { success: false, error: "Unauthorized" };
    ...
    if (action === "delete-permanently") {
      const employeeCount = await prisma.employee.count({
        where: { employeeTypeId: { in: ids } },
      });
      if (employeeCount > 0) {
        return { success: false, error: `Cannot delete permanently: employees are associated with some of these types.` };
      }
