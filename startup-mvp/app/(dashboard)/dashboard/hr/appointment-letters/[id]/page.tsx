...
export default function AppointmentLetterDetailPage({
  ...
  useEffect(() => {
    getAppointmentLetterById(resolvedParams.id)
      .then((res) => {
  ...
  if (loading) {
    return <div className="p-12 text-center text-muted-foreground">Loading appointment letter...</div>;
  }

  if (!letterData) {
    return (
      <div className="p-12 text-center space-y-4">
        <p className="text-red-500 font-semibold">Appointment letter not found.</p>
        <Button onClick={() => router.push("/dashboard/hr/appointment-letters")}>
          Back to Appointment Letters
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 p-4 md:p-8">
      ...
      <div className="max-w-3xl mx-auto flex items-center justify-between mb-6 print:hidden">
        <Button
          variant="outline"
          onClick={() => router.push("/dashboard/hr/appointment-letters")}
      ...
      <div className="p-8 md:p-12 bg-white text-slate-900 mx-auto max-w-3xl rounded-lg shadow-lg print:shadow-none print:m-0 print:p-8 print:w-full print:max-w-none font-sans leading-relaxed">
        {/* Header / Letterhead */}
