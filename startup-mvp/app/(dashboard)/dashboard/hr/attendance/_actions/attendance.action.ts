 ...
 */
export async function logAttendancePunch(employeeId: string, timestamp: Date, source: AttendanceSource, deviceId?: string) {
  try {
...
export async function processManualAttendance(input: {
  ...
  try {
    ...
    if (attendance) {
      ...
      await logItemUpdated(session.user.id, "Attendance", attendance.id, undefined, `Attendance for ${employee.name}`, { oldAttendance, attendance });
    } else {
      // Create
