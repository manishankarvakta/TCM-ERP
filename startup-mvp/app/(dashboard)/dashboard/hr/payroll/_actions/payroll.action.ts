...
export async function updatePayrollStatus(id: string, status: PayrollStatus) {
  try {
    ...
    await logItemUpdated(session.user.id, "Payroll", id, undefined, `Status changed to ${status}`, { oldPayroll, updated });
...
export async function postPayroll(payrollId: string, salaryExpenseAccountId: string) {
  try {
    ...
    const canPost = await hasPermission(session.user.id, "hr.payroll", "post" as any);
