...
export default async function MfsLedgerPage({ searchParams }: MfsLedgerPageProps) {
  ...
  // Get ledger data
  const ledgerResult = await getMfsLedger({
    ...
    <PageGuard permissionKey="accounts.cash-bank">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold">Mobile Financial Services (MFS) Ledger</h1>
            <p className="text-sm text-muted-foreground">
              Track MFS wallet transaction histories
            </p>
          </div>
        </div>
        ...
        {!canView ? (
          ...
          <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            <p className="text-sm text-destructive">
              {ledgerResult.error || "Failed to load MFS ledger"}
            </p>
          </div>
        ) : (
          <CashBankLedgerView
