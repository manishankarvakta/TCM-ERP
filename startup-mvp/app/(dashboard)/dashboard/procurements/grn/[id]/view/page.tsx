  ...
  return (
    <div className="space-y-6 print:space-y-3">
        ...
        <CardContent className="print:p-1.5">
          ...
          {grn.notes && (
            <div className="mt-4 print:mt-2 text-left">
              <p className="text-xs font-semibold uppercase text-slate-500">Note / Remarks:</p>
              {grn.notes.includes("<") ? (
                <div
                ...
                <p className="text-sm print:text-xs text-slate-700 mt-1 whitespace-pre-wrap">{grn.notes}</p>
              )}
            </div>
          )}
        </CardContent>
      ...
      <div className="hidden print:block mt-6 text-center text-[10px] text-slate-400 pt-2 border-t border-slate-100">
        <p>Generated by {org?.name || "TCM"} ERP on {format(new Date(), "PPpp")}</p>
      </div>
    </div>
  );
}
