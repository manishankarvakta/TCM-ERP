...
import { FiSearch, FiEdit, FiTrash2, FiX, FiCircle, FiCheck, FiMoreVertical, FiEye, FiRotateCw, FiUser } from "react-icons/fi";
...
export default function ClientsListClient({
  ...
  useEffect(() => {
    // Skip if search matches the URL param (initial load or manual sync)
    ...
    if (tab) {
        // Param already preserved via toString() but just ensuring logic consistency
    }
    router.push(`/dashboard/crm/clients?${params.toString()}`, { scroll: false });
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Table */}
      {initialClients.length === 0 && !search && !isTrash ? (
         <div className="border rounded-lg p-12 flex justify-center bg-muted/5 border-dashed">
            <EmptyState 
                icon={FiUser} 
              ...
              <TableRow>
                <TableCell colSpan={8} className="h-24 text-center">
                    {search ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-6 text-muted-foreground">
                            <p>{isTrash ? "No trashed clients found" : "No clients found"}</p>
                        </div>
                    )}
                </TableCell>
              </TableRow>
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => {
                ...
                router.push(`/dashboard/crm/clients?${params.toString()}`);
              }}
            >
              Previous
            </Button>
