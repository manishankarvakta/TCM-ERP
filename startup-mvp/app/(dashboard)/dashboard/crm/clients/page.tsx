...
export default async function ClientsPage({ searchParams }: ClientsPageProps) {
  ...
  const [result, canView, canEdit, canMoveToTrash, canDeletePermanently, canCreate] = await Promise.all([
    ...
    userId ? hasPermission(userId, "peoples.clients", "create") : false,
  ]);
    ...
    <PageGuard permissionKey="peoples.clients">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {tab !== "trash" && canCreate && (
            <Button asChild>
              <Link href="/dashboard/crm/clients/add">
        ...
        <Tabs defaultValue={tab} className="w-full">
          <TabsList>
            <TabsTrigger value="all" asChild>
              <Link href="/dashboard/crm/clients?tab=all&page=1">All Clients</Link>
            </TabsTrigger>
            <TabsTrigger value="trash" asChild>
              <Link href="/dashboard/crm/clients?tab=trash&page=1">Trash</Link>
            </TabsTrigger>
          </TabsList>
