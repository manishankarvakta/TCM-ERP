...
export default function OrganizationsListClient({
  ...
  const handleSearch = (value: string) => {
    ...
    router.push(`/admin/settings?section=organization&${params.toString()}`);
  };
                        ...
                        {!isTrash && (
                          <>
                            <Button variant="ghost" size="sm" asChild>
                              <Link href={`/admin/settings?section=organization&view=${org.id}`}>
                                <FiEye className="h-4 w-4" />
                              </Link>
                            </Button>
                            <Button variant="ghost" size="sm" asChild>
                              <Link href={`/admin/settings?section=organization&edit=${org.id}`}>
              ...
              onClick={async () => {
                if (isTrash && deleteOrganizationId) {
                  ...
                  if (result.success) {
                    ...
                    const currentUrl = `/admin/settings?section=organization${searchParams.get("tab") ? `&tab=${searchParams.get("tab")}` : ""}${searchParams.get("page") ? `&page=${searchParams.get("page")}` : ""}`;
