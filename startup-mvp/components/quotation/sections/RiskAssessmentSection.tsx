  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {!readOnly && <Button type="button" variant="outline" size="sm" onClick={addRisk}><Plus className="h-3.5 w-3.5 mr-1.5" />Add Risk</Button>}
      </div>

      {risks.map((r, idx) => (
        <div key={r.id} className="rounded-lg border bg-muted/20 p-4 space-y-3">
          <div className="flex items-start gap-2">
            <Input value={r.risk} readOnly={readOnly} placeholder="Risk description"
          ...
          <div className="grid grid-cols-3 gap-3">
            {(['probability', 'impact'] as const).map((field) => (
              <div key={field} className="space-y-1 col-span-1">
                ...
                {readOnly ? (
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${LEVEL_COLOR[r[field]]}`}>{r[field]}</span>
                ) : (
            ...
            <div className="space-y-1">
              ...
              <Input value={r.owner ?? ''} readOnly={readOnly} placeholder="Risk owner"
          ...
          <div className="space-y-1">
            ...
            <Textarea value={r.mitigation} readOnly={readOnly} rows={2} placeholder="How this risk will be mitigated…"
              className="resize-none text-sm" onChange={(e) => update(idx, { mitigation: e.target.value })} />
          </div>
        </div>
      ))}
      {risks.length === 0 && !readOnly && (
